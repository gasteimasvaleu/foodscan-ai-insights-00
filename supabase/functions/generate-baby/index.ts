...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Converter URLs (signed URLs do bucket privado) para data:base64
    const toData = async (img: string) => img.startsWith('data:') ? img : await urlToDataUrl(img);
    const [motherData, fatherData] = await Promise.all([toData(motherImage), toData(fatherImage)]);
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: 'image_url', image_url: { url: motherData } },
            { type: 'image_url', image_url: { url: fatherData } },
          ],
        }],
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: `Erro ao gerar imagem (${response.status}): ${text.slice(0, 300)}` }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
