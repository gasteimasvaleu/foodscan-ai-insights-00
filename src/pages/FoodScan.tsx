...
const FoodScan = () => {
  ...
  if (loading) {
    return (
      <>
        <Navbar />
        <div className="min-h-screen bg-gradient-primary font-inter pt-[calc(env(safe-area-inset-top)+2.5rem)] pb-28 flex items-center justify-center">
  ...
  const extractElements = (data: any): FoodElement[] | undefined => {
    ...
    if (data && typeof data === 'object' && data.elementos && Array.isArray(data.elementos)) {
      ...
      return data.elementos.map((elemento: any) => ({
        ...
        estimated_weight: elemento.estimated_weight ? parseNutritionValue(elemento.estimated_weight) : undefined,
        nutrition: {
