...
export default function Comunidade() {
  ...
  const handleLike = async (postId: string) => {
    // Para simplificar, apenas incrementamos o contador
    const { error } = await supabase
      .from('post_likes')
  ...
  const handleComment = async (postId: string) => {
    ...
    const commentText = `${commenterName}: ${newComment}`;
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      placeholder="Conte sua história de transformação, seus desafios e conquistas..."
                  ...
                  <Card key={post.id} className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 border-0 bg-gradient-to-br from-white to-primary/5">
                    <CardContent className="p-6">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t">
                        ...
                        <span className="text-sm text-muted-foreground">#{post.id.slice(-6)}</span>
                      </div>
                    </CardContent>
                  </Card>
                ))
