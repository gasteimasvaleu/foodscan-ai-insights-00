...
const DailyControl = () => {
  ...
  // Hydration real data for widget sync
  const [hydrationMl, setHydrationMl] = useState(0);
  const [hydrationTarget, setHydrationTarget] = useState(3000);

  // Sync data to iOS widget via App Groups
  useWidgetSync({ goals, meals, hydrationTotals, hydrationMl, hydrationTarget });
  ...
  if (authLoading || isLoading) {
    return <>
        <Navbar />
        <div className="min-h-screen bg-gradient-primary font-inter pt-[calc(env(safe-area-inset-top)+2.5rem)] pb-28">
      ...
      <div className="min-h-screen bg-gradient-primary pt-[calc(env(safe-area-inset-top)+2.5rem)] pb-28">
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            {goals ? (
                <DailyGoals goals={goals} meals={meals} hydrationTotals={hydrationTotals} onEditGoals={handleEditGoals} onOpenAIWizard={() => setShowAIWizard(true)} />
            ) : (
