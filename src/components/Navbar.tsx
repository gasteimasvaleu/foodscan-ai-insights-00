            ...
            <SheetContent side="right" className="w-80 bg-white/10 backdrop-blur-xl border-l border-white/20 p-0">
              ...
              <ScrollArea className="h-full">
                <div className="p-6 space-y-4">
                  {menuItems.map(item => <Link key={item.label} to={item.href} className={`block p-4 rounded-xl transition-all duration-200 font-medium backdrop-blur-sm ${isActiveRoute(item.href) ? 'bg-primary-500/20 text-white border border-primary-300/30 shadow-lg' : 'bg-white/20 text-white hover:bg-white/30 hover:text-white border border-white/30'}`} onClick={() => setIsOpen(false)}>
                      <div className="flex items-center justify-between">
                        <span className="text-base text-white">{item.label}</span>
                        {isActiveRoute(item.href) && <div className="w-2 h-2 bg-primary-500 rounded-full"></div>}
                      </div>
                    </Link>)}
                </div>
              </ScrollArea>
